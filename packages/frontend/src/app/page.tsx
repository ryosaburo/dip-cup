...
export default function TopPage() {
  ...
  const { state, createRoom, joinRoom, findMatch, cancelMatchmaking, clearError } = useGameSocket();
    ...
    <main className="flex-1 flex items-center justify-center p-6">
      <div className="w-full max-w-md space-y-6">
        ...
        {state.phase === "matchmaking" ? (
          <section className="pop-panel p-4 space-y-3 text-center">
            <h2 className="font-bold text-[var(--pop-ink)]">対戦相手を探しています…</h2>
            <p className="text-sm text-[var(--pop-ink-soft)] animate-pulse py-2">
              他のプレイヤーの参加をお待ちください
            </p>
            <button
              onClick={cancelMatchmaking}
              className="pop-bounce w-full rounded-full border-2 border-[var(--pop-ink)]/20 text-[var(--pop-ink-soft)] font-bold py-2.5"
            >
              キャンセル
            </button>
          </section>
          ...
          <>
            <section className="pop-panel p-4 space-y-3">
              <h2 className="font-bold text-[var(--pop-ink)]">自動マッチング</h2>
              <p className="text-xs text-[var(--pop-ink-soft)]">
                ボタン一つで、待っている他のプレイヤーと自動的に対戦できます
              </p>
              <button
                disabled={!playerName.trim()}
                onClick={() => findMatch(playerName.trim(), session?.access_token)}
                className="pop-bounce w-full rounded-full bg-gradient-to-r from-emerald-300 to-teal-400 text-white font-bold py-2.5 shadow-[0_5px_0_rgba(150,120,200,0.35)] disabled:opacity-40 disabled:shadow-none"
              >
                対戦相手を探す
              </button>
            </section>
